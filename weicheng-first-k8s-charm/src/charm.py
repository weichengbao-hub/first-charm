...
class WeichengFirstK8SCharmCharm(ops.CharmBase):
    """Charm the application."""

    def __init__(self, framework: ops.Framework):
        super().__init__(framework)
        framework.observe(self.on["web"].pebble_ready, self._on_pebble_ready)
        self.container = self.unit.get_container("web")

    def _on_pebble_ready(self, event: ops.PebbleReadyEvent):
        """Handle pebble-ready event."""
        logger. info("Pebble is ready, starting nginx")
        ...
        layer: ops.pebble.LayerDict = {
            "services": {
                SERVICE_NAME: {
                    ...
                    "command": "nginx -g 'daemon off;'",  # Change this!
                    "startup": "enabled",
                }
            }
        }
        self.container.add_layer("nginx", layer, combine=True)
        ...
        self.wait_for_ready()
        version = weicheng_first.get_version()
...
if __name__ == "__main__":  # pragma: nocover
    ops.main(WeichengFirstK8SCharmCharm)
